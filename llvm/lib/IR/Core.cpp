...
void LLVMMarkFieldProjection(LLVMValueRef Inst, size_t Idx){
  ...
  cast<Instruction>(Val)->setMetadata("FieldProjection", N);
}
...
void LLVMSetSmartPointerHouseMetadata(LLVMValueRef Alloca){
  ...
  unwrap<Instruction>(Alloca)->setMetadata("RustMeta-Smart-Pointer-House", N);
}
...
LLVMValueRef LLVMRustMetaGetSmartPointerProjection(LLVMValueRef Val) {
  ...
  Address2PhiMap.insert(std::make_pair(Address, (Instruction*)dummy));
  return wrap(dummy);
  ...
  Value* dummyMask = IRB.CreateAnd(std::vector<Value*>({AddrToInt, ConstantInt::get(Type::getInt64Ty(context), -1)}));
  ...
  Value* addressLower = IRB.CreateAnd(std::vector<Value*>({AddrToInt, ConstantInt::get(Type::getInt64Ty(context), lowerAddrOffsetMask)}));
  Value* OptLoadAddress = IRB.CreateOr(std::vector<Value*>({shadowAddrInt, addressLower}));
...
void LLVMMarkExchangeMallocFunc(LLVMValueRef Fn){
  Function* F = unwrap<Function>(Fn);
  LLVMContext &C = F->getContext();
  MDNode *N = MDNode::get(C, MDString::get(C, "Is Exchange Malloc Func"));
